...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b
  (<*>) (State f) (State g) = State (\s -> let (a, u) = g t
                                               (h, t) = f s
                                           in (h a, u))
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat lst = eval (findM predf lst) S.empty
  where predf a = State (\s -> (S.member a s, S.insert a s))
...
distinct ::
  ...
  List a
  -> List a
distinct xs = eval (filtering predf xs) S.empty
  where predf el = State (\s -> (S.notMember el s, S.insert el s))
-- distinct xs = eval (filtering predf xs) S.empty
--   where
--     predf el = State (\s -> (S.notMember el s, S.insert el s))
